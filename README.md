...
# 3) 填凭据：复制 .env.example 为 .env，填上你的 Canvas 地址与访问令牌
cp .env.example .env
$EDITOR .env
...
│   ├── cli.py               # canvas 命令：doctor/courses/files/download/manifest/deadlines
...
- `uv run --dev pytest` 共 22 项测试，**全部离线**：不联网、不需要令牌、不需要任何 secret，
